...
const Admin = (): React.ReactElement => {
  ...
  // METHODS
  const formatChurn = (value: number): string => {
        ...
        <div className="mx-auto max-w-4xl flex flex-col gap-6">
          <div className="border border-gray-700 rounded-lg p-6">
            ...
            <p className="text-gray-200 flex items-center gap-2">
              ...
              <QuestionMarkCircleIcon className="w-5" title="Classified as dropped -> last entry > 14 days ago" />
            </p>
            <p className="text-gray-200">Average entries before churn: {churn ? churn.avgEntriesBeforeChurn : 'Loading...'}</p>
          </div>
          <div className="border border-gray-700 rounded-lg p-6">
            <p className="text-xl fond-bold mb-4">Retention Rate</p>
            <p className="text-gray-200">7 days: {retention ? formatChurn(retention.retention[7]) : 'Loading...'}</p>
            <p className="text-gray-200">14 days: {retention ? formatChurn(retention.retention[14]) : 'Loading...'}</p>
            <p className="text-gray-200">21 days: {retention ? formatChurn(retention.retention[21]) : 'Loading...'}</p>
            <p className="text-gray-200">30 days: {retention ? formatChurn(retention.retention[30]) : 'Loading...'}</p>
            <p className="text-gray-200">60 days: {retention ? formatChurn(retention.retention[60]) : 'Loading...'}</p>
          </div>
        </div>
      </div>
    </div>
